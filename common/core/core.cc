...
Core::countInstructions(IntPtr address, UInt32 count)
{
   ...
   //added by swain
#ifdef _INST_TRACE_
   if (trace_manager::is_roi()) {
      for (UInt32 i = 0; i < count; i++) {
         bool is_roi = trace_manager::getSingleton()->is_thread_roi(this->m_core_id);
         trace_manager::getSingleton()->add_record(inst_record(atol(itostr(m_performance_model->getElapsedTime()).c_str()), this->m_core_id, is_roi));
      }
   }
#endif
